...
dataframe = pd.read_csv("./gold_data_2015_25.csv")
...
if len(numeric_cols) > 0:
    ...
    low, high = filtered_dataframe[num_col].quantile([0.01, 0.99])
    filtered_dataframe = filtered_dataframe.loc[
        filtered_dataframe[num_col].notna() & (filtered_dataframe[num_col].between(low, high))
    ]
print("\nFiltered data set")
...
annual = (
    dataframe.dropna(subset=["GLD"])
      .groupby(dataframe["Date"].dt.year)
...
if len(numeric_cols) >= 2:
    ...
    reg_df = filtered_dataframe[[x_col, y_col]].dropna()
